...
class Lifecycle:
    ...
    def create(self, operation_id: str, when: datetime, size: int, name: str) -> Path:
        ...
        self._write(
            operation_id,
            {
                ...
                "state": "RETAINED",
                "quarantined_at": when.astimezone(UTC).isoformat(),
                "byte_count": size,
                "file_count": 1,
                "export_count": 0,
            },
    ...
    def records(self) -> list[Record]:
        ...
        for path in sorted(self.failed.iterdir()):
            ...
            result.append(
                Record(
                    path.name,
                    datetime.fromisoformat(str(metadata["quarantined_at"])).astimezone(UTC),
                    size,
                )
            )
    ...
    def resume(self, now: datetime) -> int:
        ...
        for operation_id in sorted(pending):
            ...
            metadata.update(
                {
                    "state": "DISPOSED",
                    "disposed_at": now.astimezone(UTC).isoformat(),
                }
            )
    ...
    def reconcile(
        ...
        *,
        ...
        protected.update(
            record.operation_id
            for record in sorted(records, key=lambda item: item.quarantined_at, reverse=True)[
                : policy.minimum
            ]
        )
        ...
        def eligible() -> list[Record]:
            return [
                ...
                if record.operation_id not in protected and len(retained) > policy.minimum
            ]
        ...
        for record in disposed:
            ...
            self._write(record.operation_id, metadata)
        return (
            {record.operation_id for record in retained},
            {record.operation_id for record in disposed},
        )
    ...
    def export(self, operation_id: str, destination: Path, *, approved: bool) -> str:
        ...
        metadata.update(
            {
                "export_count": int(metadata.get("export_count", 0)) + 1,
                "last_export_sha256": digest,
                "last_export_bytes": len(raw),
            }
        )
        self._write(operation_id, metadata)
        return digest
