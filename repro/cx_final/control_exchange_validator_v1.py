...
def rule_004(b: Bundle) -> list[str]:
    ...
    if generation == 0:
        ok = (
            pred is None
            and c.get("predecessor_claim_id") is None
            and c.get("claim_basis") == "INITIAL_CLAIM"
        )
    else:
        ok = (
...
def rule_006(b: Bundle) -> list[str]:
    ...
    ok = renewal.get("renewal_id") == expected_id and all(
        renewal.get(a) == claim.get(c) for a, c in fields
    )
...
def rule_007(b: Bundle) -> list[str]:
    ...
    expected_previous = (
        claim.get("lease_expires_at") if prior is None else prior.get("new_lease_expires_at")
    )
    try:
        ok = (
...
def rule_008(b: Bundle) -> list[str]:
    ...
    try:
        effective_expiry = parse_time(
            b.context.get("claim_effective_lease_expires_at", claim["lease_expires_at"])
        )
...
def rule_010(b: Bundle) -> list[str]:
    for alias in b.context.get("authorization_aliases", ["claim", "resource_lock"]):
        ...
        if record.get("takeover_basis") == "OWNER_DIRECTED":
            if not isinstance(auth, dict) or not re.fullmatch(
                r"[0-9a-f]{64}", str(auth.get("sha256", ""))
            ):
...
def _resource_ok(record: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and record.get("lock_head_path")
        == f"docs/coordination/control_exchange/resource_lock_heads/{digest}.json"
    )
...
def rule_012(b: Bundle) -> list[str]:
    ...
    if state == "HELD":
        ...
        ok = head.get("effective_lease_expires_at") is None and isinstance(
            head.get("release"), dict
        )
    else:
        ok = False
...
def rule_013(b: Bundle) -> list[str]:
    ...
    expected_sequence = 1 if prior is None else prior.get("renewal_sequence", -1) + 1
    expected_previous = (
        acq.get("initial_lease_expires_at") if prior is None else prior.get("new_lease_expires_at")
    )
    same = all(
    ...
    try:
        time_ok = (
            ...
            and parse_time(renewal["new_lease_expires_at"])
            > parse_time(renewal["previous_lease_expires_at"])
        )
    except (ValidationFailure, KeyError):
        time_ok = False
...
def rule_014(b: Bundle) -> list[str]:
    ...
    if generation == 0:
        ok = (
            pred is None
            and lock.get("predecessor_lock") is None
            and lock.get("takeover_basis") == "INITIAL_ACQUISITION"
        )
    else:
        ...
        if basis == "RELEASED_REACQUISITION":
            ...
            try:
                ...
                ok = (
                    ok
                    and pred_head.get("effective_state") == "HELD"
                    and decision >= expiry + timedelta(seconds=120)
                )
            except (ValidationFailure, KeyError):
                ok = False
...
def rule_015(b: Bundle) -> list[str]:
    ...
    return (
        []
        if all(result.get(a) == claim.get(c) for a, c in fields)
        else [error("CX-SV-015", "STALE_OR_MISMATCHED_CLAIM")]
    )
...
def rule_017(b: Bundle) -> list[str]:
    ...
    ok = (
        mapping.get("legacy_content_sha256") == expected
        and mapping.get("carrier_sha256") == expected
    )
...
def rule_019(b: Bundle) -> list[str]:
    ...
    for alias in aliases:
        ack = b.record(alias)
        ref = next(
            (x for x in marker.get("cutover_acknowledgements", []) if x.get("alias") == alias), None
        )
...
def rule_020(b: Bundle) -> list[str]:
    ...
    if state in {"ABSENT", "FREE"}:
        ...
        ok = (disposition == "HELD_BLOCKS_CUTOVER" and blocks is True) or (
...
def _barrier_common(b: Bundle, marker_alias: str, transaction_type: str) -> bool:
    ...
    if evidence.get("component_lock_count") != len(locks) or evidence.get("readback_count") != len(
        reads
    ):
        return False
    ...
    for lock in locks:
        ...
        if lock.get("state") != "HELD":
            return False
    for read in reads:
        if read.get("matched") is not True or read.get("expected_sha256") != read.get(
            "observed_sha256"
        ):
            return False
...
def rule_021(b: Bundle) -> list[str]:
    return (
        []
        if _barrier_common(b, "effective_cutover_marker", "CUTOVER")
        else [error("CX-SV-021", "CUTOVER_BARRIER_INVALID")]
    )
...
def rule_022(b: Bundle) -> list[str]:
    ...
    for alias in aliases:
        ack = b.record(alias)
        ref = next(
            (x for x in marker.get("rollback_acknowledgements", []) if x.get("alias") == alias),
            None,
        )
        if not isinstance(ref, dict) or ref.get("sha256") != b.raw_sha(alias):
...
def rule_023(b: Bundle) -> list[str]:
    ...
    allowed = {
        "COMPLETE_UNDER_RECORDED_PROCEDURE",
        "RECOVER_UNDER_RECORDED_PROCEDURE",
        "QUARANTINE_CONFLICT",
    }
    for item in claims:
        if item.get("disposition") not in allowed:
            return [error("CX-SV-023", "INVALID_DISPOSITION")]
        if item.get("disposition") == "COMPLETE_UNDER_RECORDED_PROCEDURE" and not item.get(
            "terminal_result_sha256"
        ):
            return [error("CX-SV-023", "MISSING_TERMINAL_RESULT")]
        if item.get("disposition") == "RECOVER_UNDER_RECORDED_PROCEDURE" and not item.get(
...
def materialize_case(corpus: dict[str, Any], case: dict[str, Any]) -> dict[str, Any]:
    ...
    raw_records = {
        alias: canonical_json(record) for alias, record in base.get("records", {}).items()
    }
    return {
        "bundle_version": "1.0",
        "case_id": case["id"],
...
def run_corpus(corpus: dict[str, Any]) -> dict[str, Any]:
    ...
    for case in corpus.get("cases", []):
        ...
        case_results.append(
            {"id": case["id"], "pass": ok, "observed": observed, "expected": expected}
        )
        passed += int(ok)
    return {
        "validator_version": VERSION,
