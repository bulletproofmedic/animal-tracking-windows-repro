...
def main():
 with tempfile.TemporaryDirectory(prefix='cf004-v5-') as td:
  r=Path(td)/'repo';r.mkdir();run(r,'git','init','-q');
  ...
  checkout(r,base);(r/'side.txt').write_text('side\n');side=commit(r,'side');t=out(r,'git','rev-parse',f'{valid}^{{tree}}');env=os.environ.copy();env.update({'GIT_AUTHOR_NAME':'D','GIT_AUTHOR_EMAIL':'d@e.invalid','GIT_COMMITTER_NAME':'D','GIT_COMMITTER_EMAIL':'d@e.invalid'});merge=subprocess.check_output(['git','commit-tree',t,'-p',valid,'-p',side,'-m','merge'],cwd=r,text=True,env=env).strip();checkout(r,merge);(r/MANIFEST).write_text(valid_text);rec('true_two_parent_merge_ref','PASS',validate(r,json.loads(valid_text),base,STATE,'merge-ref',valid,r/MANIFEST));checkout(r,valid);rec('merge_ref_on_exact_head','FAIL',validate(r,json.loads(valid_text),base,STATE,'merge-ref',valid,r/MANIFEST))
