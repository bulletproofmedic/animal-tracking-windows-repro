...
class FailedRootLifecycleTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for operation_id, days, size in zip(ids, (20, 10, 2, 1), (7, 7, 7, 7), strict=True):
                lifecycle.create(
                    ...
                    size,
                    "sensitive-location.bin",
                )

            retained, disposed = lifecycle.reconcile(
                Policy(minimum=1, max_count=3, max_bytes=15, max_age_days=5),
                now,
            )
            ...
            self.assertLessEqual(
                sum((lifecycle.failed / item / "sensitive-location.bin").stat().st_size for item in retained),
                15,
            )
    ...
    def test_current_authority_and_newest_forensic_root_are_protected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            retained, disposed = lifecycle.reconcile(
                Policy(minimum=1, max_count=1, max_bytes=5, max_age_days=1),
                now,
                protected={current},
            )
    ...
    def test_authority_guard_prevents_all_pruning(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            retained, disposed = lifecycle.reconcile(
                Policy(minimum=1, max_count=1, max_bytes=1, max_age_days=1),
                now,
