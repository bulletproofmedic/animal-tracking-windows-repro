    ...
    def test_real_wal_only_schema_and_shm_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                connection.execute("CREATE TABLE wal_only_schema (id INTEGER PRIMARY KEY, value TEXT)")
    ...
    def test_checkpointed_main_database_is_accepted(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with closing(sqlite3.connect(f"file:{database}?mode=ro&immutable=1", uri=True)) as connection:
                self.assertEqual(connection.execute("SELECT value FROM sample").fetchone(), ("canonical",))

    def test_mixed_and_truncated_sidecars_never_reach_semantic_validation(self) -> None:
        variants = [f"{DATABASE_PATH}-wal", f"{DATABASE_PATH}-shm", f"{DATABASE_PATH}-journal"]
        with self.assertRaisesRegex(ValueError, "transient SQLite sidecar"):
