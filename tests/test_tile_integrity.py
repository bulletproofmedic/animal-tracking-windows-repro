...
def make_fixture(root: Path, tiles: dict[str, bytes]) -> tuple[Path, int]:
    ...
    for relative, content in sorted(tiles.items()):
        ...
        records.append({"path": relative, "sha256": hashlib.sha256(content).hexdigest(), "byte_count": len(content)})
        total += len(content)
...
class TileIntegrityTests(unittest.TestCase):
    def test_exact_closure_freezes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            manifest, total = make_fixture(root, {"0/0/0.png": b"zero", "0/1/0.png": b"one"})
    ...
    def test_add_remove_preserving_total_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            manifest, total = make_fixture(root, {"0/0/0.png": b"AAAA", "0/1/0.png": b"BBBB"})
    ...
    def test_mutation_after_copy_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "digest closure mismatch"):
                freeze(manifest, root / "maps", root / "frozen", total, mutate_after_copy=lambda: tile.write_bytes(b"BBBB"))
    ...
    def test_windows_junction_parent_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(["cmd", "/c", "mklink", "/J", str(level), str(external)], capture_output=True, text=True, check=False)
